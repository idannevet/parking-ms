...
const NAV = [
  ...
  { href: '/lots',          icon: Building2,        label: 'Parking Lots' },
  ...
  { href: '/subscriptions', icon: TicketCheck,      label: 'Subscriptions' },
];

export default function Sidebar() {
  const path = usePathname();
  return (
    <aside className="fixed left-0 top-0 h-full w-60 bg-[#0A0F1A] border-r border-[#1F2937] flex flex-col z-40">
      {/* Logo */}
      <div className="px-5 py-5 border-b border-[#1F2937]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-[10px]">Smart City Parking</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ...
        <div className="pt-3 mt-2 border-t border-[#1F2937]">
          <Link href="/ai-console"
            ...
            <Sparkles size={17} />
            AI Query Console
            <span className="ml-auto text-[9px] font-bold bg-purple-600/20 text-purple-400 px-1.5 py-0.5 rounded-full">AI</span>
          </Link>
        </div>
      </nav>
